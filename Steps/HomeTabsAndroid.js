...
When(/^I see the home screen$/, async () => {  
    ...
    var homeScreenTitle = await driver.findElements(By.xpath("//android.widget.TextView[@text=\"Testing instructions\"]"));
...
Then(/^I see Settings and Dashboard tabs are loaded$/, async () => {
    ...
    var settingTab = await driver.findElements(By.xpath("//android.view.View[contains(@resource-id,\"Settings tab\")]"));
    assert.equal(settingTab.length,1)


    var dashboardTab = await driver.findElements(By.xpath("//android.view.View[contains(@resource-id,\"Dashboard tab\")]"));
